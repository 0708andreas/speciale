...
\begin{remark}\label{rem:CGS_CGB_together}\upshape
  ...
  This also means, that by following the reduction procedure of theorem~\ref{thm:reduce_grb_system}, we can compute reduced Gröbner segments simultanously with fathful Gröbner segments. Furthermore, by performing the same reduction process on $G'$ as we do on $\{\sigma^{1}(g) - \sigma^{0}(g) \mid g \in G'\}$, we can compute a faithful Gröbner segment, which specializes to the reduced Gröbner basis, i.e.\ a reduced, faithful segment. In other words, if we write $G' = \{g_{1}, \dots, g_{n}\}$, then we can write $\{\sigma^{1}(g) - \sigma^{0}(g) \mid g \in G'\} = \{g'_{1}, \dots, g'_{n}\}$ such that $\sigma(g_{i}) = \sigma(g'_{i})$ for all $i$. Then, for a $g_{j} \in G'$, which we pseudo-reduce modulo $G' \setminus \{g_{j}\}$ as follows
  \[c g_{j} = r + \sum_{i \neq j} h_{i} g_{i}\]
  we can write
  \[c g'_{j} = r' + \sum_{i \neq j} h_{i} g_{i}\]
  will then satisfy $\sigma(r) = \sigma(r')$.
