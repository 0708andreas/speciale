...
\begin{proof}
  ...
  For the last assertion, if $\sigma_{\alpha}(r) = 0$, then clearly $\sigma_{\alpha}(f) \in \langle \sigma_{\alpha}(G) \rangle$. On the other hand, if $\sigma_{\alpha}(f) \in \langle \sigma_{\alpha}(G) \rangle$, then $\sigma_{\alpha}(f)$ reduces to 0 mod $\sigma_{\alpha}(G)$ since $\sigma_{\alpha}(G)$ is a Gröbner basis. Hence, by lemma~\ref{lem:div_to_ps_div}, there is a $h \in \langle G \rangle$ and $b \in A \setminus \ker(\sigma_{\alpha})$ such that $\sigma_{\alpha}(h) = \sigma_{\alpha}(bf)$. Hence, we can find a pseudo-reduction $h = \sum h_{i} g_{i}$ and hence $b f = r + h = r + \sum h_{i} g_{i}$ for some $r \in \ker(\sigma_{\alpha})$ is a pseudo-reduction with $\sigma_{\alpha}(r) = 0$.
\end{proof}
