...
\begin{example}\upshape
  ...
  \[\langle \sigma_{\p}(I) \rangle =
    ...
    \end{cases}
  \]
  which should make it clear why there is no parametric Gröbner basis for $I$ on all of $\C[u]$.
\end{example}
...
\begin{theorem}\label{thm:segs_are_para}
  Let $F \subset k[X, U]$ and $S \subset k[U]$ be finite sets of polynomials and let $\mathcal H = \mathbf{CGS}(F, S)$. If $(E, \{h\}, G) \in \mathcal H$, then $V(E) \setminus V(\{h\})$ is a parametric set and
  \[\{\frac g {\LC_{U}(g)} \mid g \in G\} \subset \mathcal O_{k[U]}[X]\]
  is its reduced Gröbner basis.
\end{theorem}
\begin{proof}
  Since $h = \lcm(\{\LC_{U}(g) \mid g \in G \setminus k[U] \})$,
\end{proof}
